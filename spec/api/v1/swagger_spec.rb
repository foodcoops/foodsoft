...
describe 'API v1', type: :apivore, order: :defined do
  ...
  context 'has valid paths' do
    ...
    context 'article_categories' do
      ...
      it { is_expected.to validate(:get, '/article_categories/{id}', 404, auth({'id' => ArticleCategory.last.id + 1})) }
    end
    ...
    context 'group_order_articles' do
      ...
      context 'in ordergroup' do
        ...
        shared_examples 'create group_order_articles' do
          ...
          context 'with insufficient apple points' do
            before { FoodsoftConfig[:use_apple_points] = true }
            before { expect_any_instance_of(Ordergroup).to receive(:not_enough_apples?).and_return(true) } # mock because conditions are complex
            it { is_expected.to validate(:post, '/group_order_articles', 403, auth({'_data' => {'group_order_article' => {'order_article_id' => oa_2.id, 'quantity' => 1}}})) }
          end
        end
        ...
        shared_examples 'modify group_order_articles' do
          ...
          context 'with insufficient apple points ' do
            before { FoodsoftConfig[:use_apple_points] = true }
            before { allow_any_instance_of(Ordergroup).to receive(:not_enough_apples?).and_return(true) } # mock because conditions are complex
            it { is_expected.to validate(:patch,  '/group_order_articles/{id}', 403, auth({'id' => goa_2.id, '_data' => {'group_order_article' => {'quantity' => 1}}})) }
            it { is_expected.to validate(:delete, '/group_order_articles/{id}', 200, auth({'id' => goa_2.id})) }
          end
        end
