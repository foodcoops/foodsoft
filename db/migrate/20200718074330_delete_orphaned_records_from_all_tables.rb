class DeleteOrphanedRecordsFromAllTables < ActiveRecord::Migration
  def up
    ...
    execute "DELETE FROM group_order_articles WHERE order_article_id NOT IN (SELECT id FROM order_articles)"
    ...
    execute "DELETE FROM message_recipients WHERE user_id NOT IN (SELECT id FROM users)"
    execute "UPDATE messages SET group_id=NULL WHERE group_id NOT IN (SELECT id FROM groups)"
    execute "DELETE FROM messages WHERE id NOT in (SELECT message_id FROM message_recipients)"
    execute "UPDATE messages m1 SET reply_to=NULL WHERE NOT EXISTS (SELECT * FROM (SELECT * FROM messages) AS m2 WHERE m1.reply_to=m2.id)"
    execute "DELETE FROM message_recipients WHERE message_id NOT IN (SELECT id FROM messages)"
    ...
    execute "DELETE FROM group_order_articles WHERE order_article_id NOT IN (SELECT id FROM order_articles)"
    execute "DELETE FROM group_order_article_quantities WHERE group_order_article_id NOT IN (SELECT id FROM group_order_articles)"
