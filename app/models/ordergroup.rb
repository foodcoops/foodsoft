...
class Ordergroup < Group
  ...
  def add_financial_transaction!(cost_type, amount, note, user)
    transaction do
      t = FinancialTransaction.new(:ordergroup => self, :cost_type => cost_type, :amount => amount, :note => note, :user => user)
      t.save!
      self.account_balance = financial_transactions.credit.sum('amount')
      save!
