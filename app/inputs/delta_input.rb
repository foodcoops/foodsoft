...
class DeltaInput < SimpleForm::Inputs::StringInput
  ...
  def input(wrapper_options)
    ...
    template.content_tag :div, class: 'delta-input input-group' do
      delta_button(template.content_tag(:i, nil, class: 'glyphicon glyphicon-minus'), -1, options) +
        @builder.text_field(attribute_name, options) +
        delta_button(template.content_tag(:i, nil, class: 'glyphicon glyphicon-plus'), 1, options)
    end
  end
  ...
  def delta_button(title, direction, options)
    ...
    template.content_tag(:div, class: 'input-group-btn') do
      template.button_tag(title, type: :button, name: 'delta', value: delta, data: data, tabindex: -1, class: 'btn btn-default')
    end
  end
end
