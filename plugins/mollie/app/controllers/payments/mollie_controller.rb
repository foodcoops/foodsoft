...
class Payments::MollieController < ApplicationController
  ...
  def result
    ...
    logger.debug "TRANSACTiON PROCESSED #{transaction.inspect}"
    update_transaction transaction
    case transaction.payment_state
    when 'paid'
      redirect_to root_path, notice: t('.controller.result.notice', amount: "#{transaction.payment_currency} #{transaction.payment_amount}")
    when 'open', 'pending'
